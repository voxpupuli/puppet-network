...
describe Puppet::Type.type(:network_route).provider(:sles) do
  def fixture_data(file)
    basedir = File.join(PROJECT_ROOT, 'spec', 'fixtures', 'provider', 'network_route', 'sles')
  ...
  describe 'when parsing' do
    ...
    describe 'an invalid file' do
      it 'fails' do
        ...
        end.to raise_error(%r{Malformed SLES route file})
      end
    end
  end
  ...
  describe 'when formatting' do
    ...
    describe 'writing the route line' do
      describe 'For standard (non-default) routes' do
        ...
        it 'writes 6 fields' do
          expect(content.scan(%r{^172.17.67.0/30 .*$}).first.split.length).to eq(6)
        end

        it 'has the correct fields appended' do
          expect(content.scan(%r{^172.17.67.0/30 .*$}).first).to include('172.17.67.0/30 172.18.6.2 - vlan200 table 200')
        end
    ...
    describe 'for default routes' do
      it 'has the correct fields appended' do
        expect(content.scan(%r{^default .*$}).first).to include('default 10.0.0.1 - eth1')
      end
